...
sig.hi <- colMeans(MCMCsamples) - colSds(MCMCsamples) * qnorm(0.75)
sig.lo <- colMeans(MCMCsamples) - colSds(MCMCsamples) * qnorm(0.25)
sig.hi[sig.hi > 0 & sig.hi != 1]
sig.lo[sig.lo < 0]

col <- MCMCsamples[, "gamma[4, 1, 1]"] - MCMCsamples[, "gamma[5, 1, 1]"]
mean(col) - sd(col) * qnorm(0.75)
mean(col) - sd(col) * qnorm(0.25)

col <- MCMCsamples[, "gamma[4, 2, 1]"] - MCMCsamples[, "gamma[5, 2, 1]"]
mean(col) - sd(col) * qnorm(0.75)
mean(col) - sd(col) * qnorm(0.25)
